...
class BaseMonteCarloTreeSearch:
    ...
    def search(self, board: list, own_player: int, step: int, deadline: float, reuse: bool = False) -> int:
        ...
        if reuse:
            ...
            self.initialize(board, own_player)
        # Extend the tree
        self.build_tree(self._search_batch, own_player)
        #self.extend_tree(2)
        # Keep extending the tree until we have time
        while time.time() < deadline:
            self.build_tree(self._search_batch, own_player)
        return self.get_best_action(own_player, step)
    ...
    def get_best_action(self, own_player, step):
        child = self.get_ucb_child(self._tree.current(), own_player)
        return self._tree.action(child)

    def build_tree(self, nr_of_iter, own_player):
