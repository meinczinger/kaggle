...
class BitBoard:
    def __init__(self, width, height, mark_in_row, player, bit_board: np.ndarray):
        ...
        self._draw = False
        self._game_overs = GameOvers.get_instance(
            self._width, self._height, self._mark_in_row
        )
        self._top_marks = {}
        # self._set_top_marks()
        self._matrix = None
    ...
    def list_to_bitboard(board: Union[np.ndarray, List[int]]) -> np.ndarray:
        ...
        bitboard_played = np.int64(
            0
        )  # 42 bit number for if board square has been played
        ...
        for n in range(len(board)):  # prange
            if board[n] != 0:
                ...
                if board[n] == 2:
                    bitboard_player |= (
                        1 << n
                    )  # mark as player 2 square, else assume p1=0 as default
    ...
    def is_end_state(self):
        return np.any(
            (self._bitboard[0] & self._bitboard[1]) & self._game_overs[:]
            == self._game_overs[:]
        ) or np.any(
            (self._bitboard[0] & ~self._bitboard[1]) & self._game_overs[:]
            == self._game_overs[:]
