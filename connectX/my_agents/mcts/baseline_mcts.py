...
class GameTree:
    ...
    def add_node(self, leaf, action, board, player):
        ...
        self._node_lookup[
            self.bitboard(self._last_used_node_id)
    ...
    def board(self, node):
        return BitBoard.create_from_bitboard(
            self._config.columns,
            self._config.rows,
            self._config.inarow,
            self.player(node),
            [self.bitboard(node)[0], self.bitboard(node)[1]],
        )
    ...
    def get_ubc_child(self, node, player, expand=True):
        ...
        if len(children) == 0:
            ...
            if player == 1:
                child = max(
                    children,
                    key=lambda c: self.value(c) / self.visited(c)
                    + factor * np.sqrt(np.log(self.visited(node)) / self.visited(c))
                    if self.visited(c) > 0
                    else float("inf"),
                )
            else:
...
class MonteCarloTreeSearch:
    ...
    def search(
        self, board: list, own_player: int, deadline: float, reuse: bool = False
    ) -> int:
