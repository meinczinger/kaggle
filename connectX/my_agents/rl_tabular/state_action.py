...
class StateAction:
    ...
    def best_action(self, state: BitBoard, possible_actions, ignore_epsilon=False):
        ...
        if len(self._actions(state_hash)) == 0:
            ...
            if not ignore_epsilon and (np.random.rand() <= self._epsilon):
                ...
                max_value = max(
                    [self._values[state_hash][a] for a in self._values[state_hash]]
                )
                return np.random.choice(
                    [
                        a
                        for a in self._values[state_hash]
    ...
    def add(
        self, state: BitBoard, action, reward, prev_state, prev_action, prev_reward
    ):
        self._epoch.append(
            {
                "state": state,
    ...
    def get_state_value(self, state: BitBoard):
        try:
            state_hash = state.hash()
            if state_hash in self._values:
                return max(
                    [self._values[state_hash][a] for a in self._values[state_hash]]
                )
            else:
        ...
        if u is not None:
            for state, value in u.items():
                for action, _ in value.items():
                    self._values[state.hash()][action] = 0.9 * (
                        self.get_value(state, action) + u[state][action]
                    )
